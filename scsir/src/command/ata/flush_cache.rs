...
const OPCODE_FLUSH_CACHE_EXT: u8 = 0xEA;
...
#[derive(Clone, Debug)]
pub struct FlushCacheExtSatCommand<'a> {
    raw: RawSatCommand<'a, NoData>,
}

impl<'a> FlushCacheExtSatCommand<'a> {
    fn new(sat: &'a ScsiSat<'a>) -> Self {
        let mut raw = sat.raw_nodata();
        raw.command(AtaProtocol::NonData, OPCODE_FLUSH_CACHE_EXT);
        Self { raw }
    }
...
impl ScsiSat<'_> {
    ...
    /// Command: FLUSH CACHE EXT (0xEA)
    pub fn flush_cache_ext(&self) -> FlushCacheExtSatCommand<'_> {
        FlushCacheExtSatCommand::new(self)
    }
}
